...
import {
    ...
    Database, AlertTriangle, WifiOff, CheckCircle2
} from 'lucide-react';

export default function BarangayLayout({ children, brgyName = "Brgy 7 (Poblacion)", alerts = [] }) {
    ...
    // This allows Laravel to pass alerts globally via Inertia's Share Middleware in the future
    ...
    const navLinks = [
        { name: 'Overview', icon: LayoutDashboard, route: '/barangay-panel/dashboard' },
        { name: 'Live Map', icon: MapIcon, route: '/barangay-panel/map' },
        { name: 'Sensor Devices', icon: RadioTower, route: '/barangay-panel/nodes' },
         { name: 'Sensor Logs', icon: Database, route: '/barangay-panel/logs' },
        { name: 'Reports', icon: LineChart, route: '/barangay-panel/reports' },
                    ...
                    <div className="flex items-center gap-2 sm:gap-4">
                            ...
                            {notificationsOpen && (
                                <div className="absolute right-0 mt-3 w-[300px] sm:w-[360px] bg-white rounded-3xl shadow-[0_10px_40px_rgba(0,0,0,0.08)] border border-stone-200 overflow-hidden z-50 animate-in slide-in-from-top-2 duration-200">
                                    <div className="px-5 py-4 border-b border-stone-100 flex justify-between items-center bg-stone-50/50">
                                        <h3 className="font-bold text-stone-800">Local Alerts</h3>
                                        {unreadCount > 0 && (
                                    ...
                                    <div className="max-h-[320px] overflow-y-auto custom-scrollbar">
                                        {displayAlerts.length > 0 ? (
                                            ...
                                            <div className="p-8 text-center flex flex-col items-center">
                                                ...
                                                <p className="text-sm font-bold text-stone-500">No recent alerts</p>
                                            </div>
                                        )}
                                    </div>

                                    <div className="p-3 bg-stone-50 border-t border-stone-100">
                                        <Link href="/barangay-panel/logs" className="block w-full text-center py-2 text-xs font-bold text-stone-600 hover:text-stone-900 transition-colors">
                        ...
                        <div className="flex items-center gap-2 sm:gap-3 p-1 pr-2">
                            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-stone-200 border border-stone-300 flex items-center justify-center overflow-hidden shrink-0">
                                <span className="text-xs sm:text-sm font-black text-stone-700">B7</span>
                            </div>
