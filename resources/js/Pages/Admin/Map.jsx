...
export default function LiveMap({ nodesData }) {
    ...
    // Simulated node data for Nasugbu
    const nodes = nodesData || [
        {
            id: 'NODE-001',
            name: 'Tumalim Sensor',
            location: 'Barangay Tumalim Hall',
            position: [14.0801, 120.7235],
            aqi: 45,
            heat_index: 34,
            status: 'Safe',
            signal: 85,
            power: 'Grid AC',
            details: 'MQ135 reading normal. No hazardous gases detected by MQ136.'
        },
        {
            id: 'NODE-002',
            name: 'Brgy 7 Sensor',
    ...
    const getCeramicPin = (node) => {
        ...
        const size = isSelected ? 'w-5 h-5' : 'w-3.5 h-3.5';
    ...
    return (
        <AdminLayout>
            <Head title="Live Sensor Map" />

            <div className="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <h1 className="text-4xl font-black text-stone-900 tracking-tight">Live Monitoring Map</h1>
                    <p className="text-stone-500 font-medium flex items-center gap-2 mt-2">
                        <MapIcon size={16} className="text-stone-400" /> Interactive municipality-wide sensor tracking
                    </p>
                </div>
            </div>

            {/* Massive Map Container */}
            <div className="bg-white rounded-[2.5rem] shadow-[0_4px_25px_rgba(0,0,0,0.03)] border border-stone-200/60 flex flex-col relative h-[calc(100vh-14rem)] min-h-[600px] overflow-hidden">
                ...
                {activeNode && (
                    <div className="absolute top-6 left-6 z-[400] w-80 bg-white/95 backdrop-blur-2xl rounded-[2rem] shadow-[0_10px_40px_rgba(0,0,0,0.08)] border border-stone-200/80 flex flex-col overflow-hidden animate-in slide-in-from-left-4 duration-300">

                        {/* Panel Header */}
                        <div className="px-6 py-5 border-b border-stone-100 flex justify-between items-start bg-stone-50/50">
                            <div>
                                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest mb-3 border ${activeNode.status === 'Danger' ? 'bg-rose-100 text-rose-700 border-rose-200' : 'bg-emerald-100 text-emerald-700 border-emerald-200'}`}>
                                    {activeNode.status === 'Danger' ? <AlertTriangle size={10} strokeWidth={3}/> : <CheckCircle2 size={10} strokeWidth={3}/>}
                                    {activeNode.status}
                                </span>
                                <h3 className="text-xl font-black text-stone-900 leading-tight">{activeNode.name}</h3>
                                <p className="text-xs font-bold text-stone-400 mt-1">{activeNode.location}</p>
                            </div>
                        ...
                        <div className="p-6 space-y-5">
                            <div className="grid grid-cols-2 gap-3">
                                <div className="bg-white rounded-2xl border border-stone-200 p-4 shadow-sm">
                                    ...
                                    <strong className={`text-3xl font-black tracking-tighter ${activeNode.aqi > 100 ? 'text-rose-600' : 'text-stone-800'}`}>{activeNode.aqi}</strong>
                                </div>
                                <div className="bg-white rounded-2xl border border-stone-200 p-4 shadow-sm">
                                    <span className="text-[10px] font-bold text-stone-400 uppercase tracking-widest block mb-1">Heat Index</span>
                                    <strong className="text-3xl font-black tracking-tighter text-amber-500">{activeNode.heat_index}°</strong>
                                </div>
                            </div>
                            ...
                            <div className="pt-4 border-t border-stone-100">
                                <span className="text-[10px] font-bold text-stone-400 uppercase tracking-widest block mb-3">Hardware Connection</span>
                                <div className="flex justify-between items-center bg-white border border-stone-200 rounded-xl p-3 shadow-sm mb-2">
                                    <div className="flex items-center gap-2">
                                        <SignalHigh size={14} className="text-blue-500" />
                                        <span className="text-xs font-bold text-stone-700">Cellular Link</span>
                                    </div>
                                    <span className="text-xs font-black text-stone-900">{activeNode.signal}%</span>
                                </div>
                                <div className="flex justify-between items-center bg-white border border-stone-200 rounded-xl p-3 shadow-sm">
                                    <div className="flex items-center gap-2">
                                        <Plug size={14} className="text-emerald-500" />
                                        <span className="text-xs font-bold text-stone-700">Power Source</span>
                                    </div>
                                    <span className="text-xs font-black text-stone-900">{activeNode.power}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="px-6 py-4 bg-stone-900">
                            <button className="w-full py-2.5 bg-white/10 hover:bg-white/20 border border-white/10 text-white rounded-xl text-xs font-bold transition-colors flex items-center justify-center gap-2">
                                <Cpu size={14} /> View Full Diagnostics
                            </button>
                        </div>
                    </div>
                )}
                ...
                <div className="w-full h-full relative z-0">
                    <MapContainer center={mapCenter} zoom={13} zoomControl={false} scrollWheelZoom={true} style={{ height: '100%', width: '100%', backgroundColor: '#F7F7F5' }}>
