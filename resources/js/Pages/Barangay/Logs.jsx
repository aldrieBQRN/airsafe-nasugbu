...
import {
    ...
    ThermometerSun, CheckCircle2, Filter, Calendar, Clock, MapPin
} from 'lucide-react';

export default function BarangayLogs({ logs, filters, brgyName }) {
     ...
     * Updates URL and data without a full page refresh.
     * Sets a 3-second lock on sync to let the user finish interacting.
     */
    const applyFilters = (newSearch, newDate, newStatus) => {
        // Lock background sync for 3 seconds
        syncPausedUntil.current = Date.now() + 3000;

        router.get(route('barangay.logs'),
    ...
    /**
     * 1-SECOND SILENT BACKGROUND SYNC
     * Matches high-frequency update logic while checking the interaction lock.
     */
    useEffect(() => {
        let isRefreshing = false;
        const dataPoller = setInterval(() => {
            // Check if sync is locked by recent user interaction
    ...
    // SEARCH DEBOUNCE: Standard 300ms delay for efficiency
    useEffect(() => {
    ...
    return (
        <BarangayLayout brgyName={brgyName || "Local Area"}>
            <Head title={`Local Logs | ${brgyName || 'TRIVORA'}`} />
            ...
            <div className="mb-6 md:mb-10 px-1">
                ...
                <p className="text-sm md:text-base text-stone-500 font-medium flex items-center gap-2 mt-2">
                    <Database size={16} className="text-stone-400 shrink-0" /> Historical trends and telemetry for {brgyName}
                </p>
            </div>
                        ...
                        <div key={log.id} className="bg-white p-5 rounded-2xl border border-stone-200 shadow-sm active:scale-[0.98] transition-all">
                            ...
                            <div className="grid grid-cols-2 gap-px bg-stone-100 rounded-xl overflow-hidden border border-stone-100 mb-4">
                                <div className="bg-white p-3">
                                    <span className="text-[9px] font-bold text-stone-400 uppercase block mb-0.5">Air Quality</span>
                ...
                <div className="hidden lg:block overflow-x-auto flex-grow">
                    {logs.data.length > 0 ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-32 text-stone-400 text-center uppercase tracking-widest">
                            ...
                            <p className="font-black text-stone-500">Telemetry history empty</p>
                        </div>
                    )}
                </div>
...
function StatusPill({ status }) {
    if (status === 'Danger') return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-black uppercase bg-rose-100 text-rose-700 border border-rose-200"><AlertTriangle size={10} strokeWidth={3} /> Danger</span>;
    if (status === 'Warning') return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-black uppercase bg-amber-100 text-amber-700 border border-amber-200"><ThermometerSun size={10} strokeWidth={3} /> Warning</span>;
    return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-black uppercase bg-emerald-100 text-emerald-700 border border-emerald-200"><CheckCircle2 size={10} strokeWidth={3} /> Safe</span>;
}